...
@dataclass
class USAMO2025ResultEntry(AbstractResultEntry):
    ...
    @classmethod
    def from_json_data(cls, data: Dict[str, Any], source_file: str = "") -> 'USAMO2025ResultEntry':
        """Create a USAMO2025ResultEntry from JSON data."""
        return cls(
...
@dataclass
class USAMO2025ResultSet(AbstractResultSet):
    """USAMO2025 dataset result set."""
    
    name: str
    directory: Path
    entries: List[USAMO2025ResultEntry] = field(default_factory=list)
    ...
    def _compute_basic_metrics(self, entries: List[USAMO2025ResultEntry]) -> Dict[str, Any]:
