...
def parse_prompt(data_item: dict) -> dict:
    ...
    if edited_rubric is not None:
        ...
        rubric = data_item.get("Grading guidelines", "")
    problem = data_item.get("Problem", "")
    # solution
    solution = data_item.get("Solution", "")
    # student's answer
    answer = data_item.get("Response", "")

    # print(user_prompt)
    return {
        'system_prompt': system_prompt,
        'user_prompt': user_prompt.format(guidelines=rubric, problem_statement=problem, solution=solution, student_answer=answer)
    }
